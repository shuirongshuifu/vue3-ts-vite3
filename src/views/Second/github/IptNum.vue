...
<template>
    <div class="m-input-number" :class="{ 'input-number-disabled': disabled }" tabindex="1"
        :style="`width: ${inputWidth};`">
        <div class="m-input-wrap">
            <span class="u-input-prefix" v-if="showPrefix">
                <slot name="prefix">{{ prefix }}</slot>
            </span>
            <input v-if="keyboard" class="u-input-number" autocomplete="off" :disabled="disabled" v-model="numValue"
                @keydown.up.prevent @keydown="onKeyboard" @change="onChange" v-bind="$attrs" />
            <input v-else autocomplete="off" class="u-input-number" @change="onChange" v-model="numValue"
                v-bind="$attrs" />
        </div>
